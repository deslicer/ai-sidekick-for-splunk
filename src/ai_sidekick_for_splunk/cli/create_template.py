...
def create_search_definition() -> dict[str, Any]:
    ...
    search["title"] = get_user_input(
        "Search title (human-readable)", search["name"].replace("_", " ").title()
    )
...
def create_phase_definition() -> dict[str, Any]:
    ...
    phase["title"] = get_user_input(
        "Phase title (human-readable)", phase["name"].replace("_", " ").title()
    )
...
def generate_template_interactively() -> dict[str, Any]:
    ...
    template["title"] = get_user_input(
        "Template title (human-readable)", template["name"].replace("_", " ").title()
    )
    ...
    template["splunk_versions"] = get_list_input(
        "Supported Splunk versions (e.g., '8.0+', '9.0+'):", 1
    )
    template["required_permissions"] = get_list_input(
    ...
    structure_choice = get_choice(
        ...
        ["Simple (single list of searches)", "Complex (multiple phases with searches)"],
...
Examples:
  ...
  ai-sidekick --create-template --from-example simple_health_check
    ...
    parser.add_argument(
        "--output-dir", help="Output directory for the template (default: current directory)"
    )

    parser.add_argument(
        "--from-example", help="Start from an existing example template (copy and modify)"
    )
    ...
    if args.from_example:
        ...
        safe_print(
            f"📝 Edit the file and then validate: uv run ai-sidekick --validate-template {output_path}"
        )
        ...
        try:
            ...
            safe_print(
                f"🔍 Searches: {len(template.get('searches', []))} direct"
                + (
                    f" + {sum(len(p.get('searches', [])) for p in template.get('phases', []))} in phases"
            ...
            safe_print(
                f"2. Test: uv run ai-sidekick --create-flow-agent test_workflow --template-file {output_path}"
            )
            safe_print(
                f"3. Use: uv run ai-sidekick --create-flow-agent my_workflow --template-file {output_path}"
            )
