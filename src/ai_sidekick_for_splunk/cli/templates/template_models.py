...
class TemplateMetadata(BaseModel):
    ...
    complexity: ComplexityLevel = Field(default=ComplexityLevel.BEGINNER, description="Complexity level")
...
class TemplateRequirements(BaseModel):
    ...
    splunk_versions: List[str] = Field(default=["8.0+", "9.0+"], description="Supported Splunk versions")
    required_permissions: List[str] = Field(default=["search"], description="Required Splunk permissions")
...
class SimpleTemplate(BaseModel):
    ...
    requirements: TemplateRequirements = Field(default_factory=TemplateRequirements, description="Requirements")
    ...
    searches: Optional[List[SearchDefinition]] = Field(default=None, description="Simple search list")
    phases: Optional[List[PhaseDefinition]] = Field(default=None, description="Complex phase definition")
    
    # Advanced options
    advanced_options: TemplateAdvancedOptions = Field(default_factory=TemplateAdvancedOptions, description="Advanced options")
    ...
    def validate_searches_or_phases(self) -> 'SimpleTemplate':
        ...
        if self.searches and self.phases:
            raise ValueError("Template cannot define both 'searches' and 'phases' - choose one approach")
            ...
            if phase.depends_on:
                for dep in phase.depends_on:
                    if dep not in phase_names:
                        raise ValueError(f"Phase '{phase.name}' depends on non-existent phase '{dep}'")
