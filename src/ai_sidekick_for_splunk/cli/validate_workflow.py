...
def validate_workflow_json(workflow_path: str, verbose: bool = False) -> tuple[bool, str]:
    ...
    try:
        ...
        try:
            ...
            if verbose:
                safe_print("✅ Pydantic validation passed")
        ...
        try:
            ...
            if verbose:
                safe_print("✅ AgentFlow loading successful")
        ...
        if verbose:
            ...
            safe_print("   Required fields: ✅ All present")

            # Check phase structure
            phases = workflow_data.get("core_phases", {})
            if not phases:
                return False, "❌ No phases defined in workflow"

            safe_print("   Phase structure: ✅ Valid")
            ...
            if total_tasks == 0:
                return False, "❌ No tasks defined in any phase"
        ...
        return (
            False,
            f"❌ Import error: {e}. Make sure you're running from the correct environment.",
        )
    except Exception as e:
...
and want to ensure they'll work seamlessly with FlowPilot agents.
    ...
    parser.add_argument("workflow_file", help="Path to the workflow JSON file to validate")
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="Only show final result (success/failure)"
    )
