...
class TemplateParser:
    ...
    def load_template_from_file(self, template_path: Path) -> SimpleTemplate:
        ...
        if not template_path.suffix.lower() in ['.yaml', '.yml']:
            raise TemplateParseError(f"Template file must have .yaml or .yml extension: {template_path}")
    ...
    def load_template_from_string(self, template_content: str, source_name: str = "string") -> SimpleTemplate:
        """
    ...
    def parse_template_data(self, template_data: Dict[str, Any], source_name: str) -> SimpleTemplate:
        """
        ...
        if not isinstance(template_data, dict):
            raise TemplateParseError(f"Template must be a YAML dictionary, got {type(template_data)}")
        ...
        try:
            ...
            logger.info(f"✅ Successfully parsed template '{template.metadata.name}' from {source_name}")
            logger.debug(f"Template has {template.get_search_count()} searches in {template.get_phase_count()} phases")
            
            return template
            
        except ValidationError as e:
            error_details = self._format_validation_errors(e)
            raise TemplateParseError(f"Template validation failed in {source_name}:\n{error_details}")
        except Exception as e:
