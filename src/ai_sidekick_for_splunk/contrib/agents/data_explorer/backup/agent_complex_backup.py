...
class QualityCheckerAgent(BaseAgent):
    ...
    async def _run_async_impl(self, invocation_context) -> AsyncGenerator[Event, None]:
        ...
        completeness_score = sum(25//len(required_fields) for field in required_fields
                                if field in insights_data and insights_data[field])
        score += completeness_score

        if completeness_score < 20:
            feedback.append("Insights lack required sections (insights, evidence, impact, implementation)")
        ...
        if collected_data:
            ...
            if data_sources >= 3:
                ...
                score += 5
                feedback.append("Insufficient data sources - need more comprehensive data collection")
        else:
        ...
        for insight in insights:
            if isinstance(insight, dict):
                ...
                if any(indicator in str(impact).lower() for indicator in
                      ['$', '%', 'reduce', 'increase', 'save', 'improve', 'hours', 'minutes']):
        ...
        if len(insights) > 0:
            ...
            score += value_score
            if value_score < 15:
                feedback.append("Insights need more quantified business value (costs, savings, time, percentages)")
        else:
        ...
        if implementation:
            ...
            if impl_score < 15:
                feedback.append("Implementation plan needs more specific SPL queries, dashboards, or alerts")
        else:
